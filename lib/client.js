 ...
 *      cannot assume
 * throws RulesEvaluationFailedError if no rules pass Aperture evaluation
 *
...
MahiClient.prototype.authorize = function authorize(opts) {
    ...
    assert.object(opts.principal.account);
    assert.optionalObject(opts.principal.user);
    assert.string(opts.action);
    assert.object(opts.resource);
    assert.arrayOfString(opts.resource.roles);
    assert.object(opts.resource.owner);
