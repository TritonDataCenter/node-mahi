...
function MahiClient(opts) {
    ...
    assert.optionalNumber(opts.maxTranslationCacheSize, 'maxTranslationCacheSize');
    assert.optionalNumber(opts.maxTranslationCacheAgeMs, 'maxTranslationCacheAgeMs');
...
MahiClient.prototype.authorize = function authorize(opts) {
    ...
    roles.forEach(function (role) {
        var name = principal.roles[role].name;
...
MahiClient.prototype.getUuid = function getUuid(opts, cb) {
    ...
    // login is translated regardless if there are other things we want to
    // translate. This saves us the round trip if we are only translating the
    // account login AND we have the translation cached.

    if (!type) {
        var cached = self.translationCache.get(sprintf('/account/%s', account));
        if (cached) {
            translations.account = cached;
            setImmediate(function () {
                cb(null, translations);
            });
            return;
